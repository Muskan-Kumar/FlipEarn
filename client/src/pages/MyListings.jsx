  ...
  return (
    <div className='px-6 md:px-16 lg:px-24 xl:px-32 pt-8'>
      ...
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center mb-8'>
        ...
        <button onClick={()=>navigate('/')} className='bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded font-medium flex items-center space-x-2 mt-4 md:mt-0'>
      ...
      {userListings.length === 0 ? (
        <div className='bg-white rounded-lg border border-gray-200 p-16 text-center'>
          ...
          <button onClick={()=> navigate("/create-listing")} className='bg-indigo-600 hver:bg-indigo-700 text-white px-6 py-2 rounded-lg font-medium'>Create First Listing</button>
        </div>
      ) : 
      (
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          {userListings.map((listing)=>(
            <div key={listing.id} className='bg-white rounded-lg border border-gray-200 hover:shadow-lg shadow-gray-200/70 transition-shadow'>
              <div className='p-6'>
                <div className='flex items-start gap-4 justify-between mb-4'>{platformIcons[listing.platform]}
                  <div className='flex-1'>
                          ...
                          <div className='invisible group-hover:visible absolute right-0 top-0 pt-4.5 z-10'>
                            <div className='bg-white text-gray-600 text-xs rounded border border-gray-200 p-2 px-3'>
                              ...
                              <button className='text-nowrap'>
                                Status : {" "}
                                <span className={
                                  listing.isCredentialSubmited ? listing.isCredentialVerified ? listing.isCredentialChanged ? "text-green-600" : "text-indigo-600" : "text-slate-600" : "text-red-600"
                                }>
                                  {listing.isCredentialSubmited ? listing.isCredentialVerified ? listing.isCredentialChanged ? "Changed" : "Verified" : "Submitted" : "Not Submitted"}
                                </span>
                              </button>
                            </div>
                          </div>
                    ...
                    <p><span>@{listing.username}</span></p>
                  </div>
                </div>
      ...
      {showWithdrawl && (
        <WithdrawModel onClose={()=> setShowCredentialSubmission(null)} />
      )}
      {/* Footer */}
      <div className='bg-white border-t border-gray-200 p-4 text-center mt-28'>
        <p className='text-sm text-gray-500'>
          © 2025 <span className='text-indigo-600'>Muskan</span>. All rights reserved.
        </p>
      </div>
    </div>
  )
