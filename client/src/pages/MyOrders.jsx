...
const MyOrders = () => {
  ...
  const copy = async(txt)=>{
    try{
      ...
      toast.success("Copie to clipboard")
    }catch(error){
            ...
            <div key={id} className='bg-white rounded-lg border border-gray-200 p-5 flex flex-col max-w-4xl'>
              ...
              <div className='flex flex-col gap-2 items-end'>
                <button onClick={()=>setExpandedId((p)=>(p === id ? null : id))} className='flex items-center gap-2 bg-white border border-gray-200 px-3 py-2 rounded hover:shadow text-sm' aria-expanded={isExpanded}>
                  {isExpanded ? (
                    <>
                    <ChevronUp className='size-4'/> Hide Credential
                    </>
                  ) : (
                    <>
                    <ChevronDown className='size-4'/> View Credential
                    </>
                  )}
                </button>
                ...
                <div className='text-xs text-gray-500 mt-2 text-right'>
                  <div>
                    Credential Purchase: {format(new Date(order.createdAt), "MMM, d, yyyy")}
                  </div>
                </div>
                        ...
                        <div key={cred.name} className='flex items-center justify-between gap-3 bg-gray-50 rounded-md p-2'>
                          ...
                          <div className='flex items-center gap-2'>
                            ...
                            <button onClick={(e)=>{
                              e.stopPropagation();
                              copy(cred.value)
                            }} className='px-2 py-1 text-xs bg-white border bordr-gray-200 rounded hover:shadow' title='Copy credential'>
