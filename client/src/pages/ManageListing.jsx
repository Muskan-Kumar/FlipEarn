...
const ManageListing = () => {
  ...
  const niches = ['lifesttyle', 'fitness', 'food', 'travel', 'tech', 'gaming', 'fashion', 'beauty', 'business', 'education', 'entertainment', 'music', 'art', 'sports', 'health', 'finance', 'other'];
        ...
        <form onSubmit={handleSubmit} className='space-y-8'>
          ...
          <Section title='Account Metrics'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mb-6'>
              ...
              <selectField label='Primary Audience Age Range' options={ageRanges} value={formData.age_ranger} onChange={(v)=> handleInputChange('age_ranger', v)} />
            </div>

            <div className='space-y-3'>
              <CheckBoxField label='Account is verified on the platform' checked={formData.verified} onChanged={(v)=> handleInputChange('verified', v)} />

              <CheckBoxField label='Account is monetized' checked={formData.monetized} onChanged={(v)=> handleInputChange('monetized', v)} />
            </div>
          </Section>
          ...
          <Section title='Screenshots & Proof'>
            <div className='border-2 border-bashed border-gray-300 rounded-lg p-6 text-center'>
              ...
              <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mb-4'>
                {formData.images.map((img, index)=>(
                  <div key={index} className='relative'>
                    ...
                    <button type='button' onChange={()=>removeImage(index)} className='absolute -top-2 -right-2 size-6 bg-red-600 text-white rounded-full hover:bg-red-700'>X</button>
                  </div>
                ))}
              </div>
            )}
